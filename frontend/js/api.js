...
class EscobaAPI {
    ...
    static async createMatch(player1, player2) {
        const response = await fetch(`${API_BASE}/matches/matches`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
    ...
    static async getMatchState(matchId, player) {
        const response = await fetch(`${API_BASE}/matches/matches/${matchId}?player=${player}`);
        return await response.json();
    }

    static async playCard(matchId, player, cardId) {
        const response = await fetch(`${API_BASE}/matches/matches/${matchId}/play`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
